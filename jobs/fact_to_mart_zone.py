"""
Fact to Mart Layer - mart_month_hour_zone_trip_metrics
Build mart table for month/hour/zone trip metrics with dimension join
"""
from pyspark.sql import DataFrame
from pyspark.sql.functions import (
    col, sum as spark_sum, avg, count, round as spark_round,
    current_timestamp, expr, when, lit
)
...
def build_mart_month_hour_zone_trip_metrics(df_fact: DataFrame, df_dim_zone: DataFrame) -> DataFrame:
    """
    Build mart_month_hour_zone_trip_metrics

    Grain: 1 row = year_month + pickup_hour + pickup_location_id

    Aggregations:
    - trip_count, total_passenger_count
    - total_distance, avg_distance
    - total_duration_min, avg_duration_min, p50_duration_min, p90_duration_min
    - total_fare_amount, total_tip_amount, total_tolls_amount, total_amount
    - avg_fare_amount, avg_tip_amount, avg_total_amount
    - fare_per_minute, fare_per_mile
    """
    print("\n--- Building mart_month_hour_zone_trip_metrics ---")
    print("Grain: year_month + pickup_hour + pickup_location_id")
    ...
    df_agg = df_fact.groupBy(
        ...
        spark_round(spark_sum("tolls_amount"), 2).alias("total_tolls_amount"),
    ...
    # Step 3: Join with dim_taxi_zone
    print("Step 3: Joining with dim_taxi_zone...")
    ...
    df_mart = df_with_derived.join(
        ...
        df_with_derived.pickup_location_id == df_dim_zone.location_id,
        "left"
    )
    ...
    # Step 5: Show sample
    print("\nSample mart_month_hour_zone_trip_metrics:")
    df_final.select(
        "year_month", "pickup_hour", "pickup_location_id", "borough", "zone",
        "trip_count", "total_distance", "avg_duration_min",
        "total_amount", "fare_per_mile"
    ).orderBy("year_month", "pickup_hour", "pickup_location_id").show(10, truncate=False)
...
def write_mart_table(df_mart: DataFrame, output_path: str):
    ...
    """
    print(f"\nWriting mart_month_hour_zone_trip_metrics to: {output_path}")
    ...
    print("mart_month_hour_zone_trip_metrics saved successfully!")
...
def build_and_write_mart_month_hour_zone_trip_metrics(
    spark, fact_path: str, dim_zone_path: str, output_path: str
) -> DataFrame:
    """
    Run mart_month_hour_zone_trip_metrics end-to-end:
    ...
    df_mart = build_mart_month_hour_zone_trip_metrics(df_fact, df_dim_zone)

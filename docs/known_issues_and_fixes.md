...
## 12. 다개월 순차 처리 시 경계 노이즈 행이 이전 달 파티션을 통째로 덮어씀 (12개월 배치에서 처음 재현)

**배경**: 2024-01~12(12개월) Baseline M을 처음으로 시도. 로그상 Raw/Cleaned/Fact 전부 "COMPLETE"로 찍혔지만, S3 실제 데이터를 대조해보니 `year=2024/month=1`~`month=11` 파티션이 각각 6~7KB(사실상 빈 것과 다름없음)였고, 가장 나중에 처리된 `month=12`만 정상 크기(72MB)였다.

**원인**: `main.py` STEP 1이 12개월을 순차 반복하며 매번 `save_to_raw_layer`로 **같은 raw_layer_path에 대해 dynamic partition overwrite**를 수행했다. TLC 원본 월별 parquet 파일은 100% 그 달로만 채워져 있지 않고, 파일당 10~40건 수준의 극소수 경계 노이즈 행(예: `2024-02` 파일 안에 실제 `pickup_datetime`이 1월인 행 몇 개)을 포함한다. dynamic overwrite는 "그 실행에서 실제로 쓰인 파티션만" 교체하는데, 하필 그 "실제로 쓰인 파티션" 안에 **이전 달의 파티션도 포함**될 수 있다는 게 맹점이었다 — `2024-02`를 처리할 때 그 파일 속 소수의 1월 행이 `year=2024/month=1`에 dynamic overwrite로 쓰이면서, 직전에 이미 써둔 진짜 1월 데이터(수백만 건)를 그 몇 건짜리 파티션으로 완전히 지워버렸다. 이게 2월→3월→...→12월까지 연쇄적으로 반복되어, 결국 가장 나중에 쓰인 12월만 온전히 남았다.

이전 실험(EXP-00~02, `docs/spark_tuning_results.md`)들은 1~4개월, 그것도 서로 인접하지 않은 달(2011-01 단독, 2011-11+12) 위주로 테스트해서 이 버그가 우연히 재현되지 않았을 뿐이었다 — **다개월을, 특히 연속된 달을 한 번에 처리하는 모든 시나리오에 잠재된 위험**이었다.

**조치**: `jobs/save_raw_layer.py`에 `save_month_to_raw_layer()` 신설. 기존처럼 `year(pickup_col)`/`month(pickup_col)`로 파생한 동적 파티션에 쓰는 대신, **소스 파일의 지정된(nominal) 연/월을 명시적 경로(`.../year=Y/month=M`)로 직접 지정해서 씀** (`df.write.mode("overwrite").parquet(explicit_path)`, `partitionBy` 미사용). 이러면 한 달을 처리할 때 그 달의 디렉터리만 건드리므로 다른 달과 절대 충돌하지 않는다. 필터링도 하지 않고 원본 파일의 모든 행을 그대로 보존하므로(경계 노이즈 행도 그 소스 파일의 nominal month 아래에 남음), "Raw 데이터는 수정하지 않는다" 원칙도 그대로 유지된다. `main.py` STEP 1 루프를 이 함수로 교체.

**검증**: 로컬에서 "2024-01 파일(5건, 전부 1월)" + "2024-02 파일(6건 = 2월 5건 + 1월 경계 노이즈 1건)"을 합성해 재현. 수정 전 코드였다면 1월 파티션이 노이즈 1건으로 덮어써졌을 상황을, 수정 후에는 1월 5건 그대로 유지 / 2월 6건(노이즈 포함, 소실 없음)으로 확인. 이후 실제 클러스터에서 12개월 재실행 → 전 달이 58~75MiB로 고르게 채워진 것으로 최종 확인.

**교훈**: dynamic partition overwrite는 "내가 쓰려는 파티션만 안전하게 건드린다"는 보장이지, "내가 의도한 파티션만 건드린다"는 보장이 아니다. 소스 데이터에 파티션 키 기준으로 완벽하게 깨끗하지 않은(경계에 걸친) 행이 조금이라도 섞여 있고, 같은 출력 경로에 여러 번 나눠서 쓴다면, 의도치 않은 파티션이 조용히 통째로 사라질 수 있다. 이번처럼 "일부 데이터가 정상 크기의 1/10000 수준"이라는 명백한 이상치가 아니었다면 로그의 "COMPLETE"만 보고 넘어갔을 수도 있다 — S3 실제 산출물의 크기/건수를 직접 대조하는 습관이 중요하다.
